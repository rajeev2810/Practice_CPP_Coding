...
{
public:
	// Modified as per new standard
    Block() = default;
    ~Block() = default;
	
	// Return and pass by reference to avoid copies
    std::string& GetName() { return m_name; }
    void SetName(const std::string& name) { m_name = name; }
	
	// Replaced push_back with emplace_back for faster execution
    void AddAttribute(const std::string& attribute) { m_attributes.emplace_back(attribute); } 
...
struct Graph
{
    ...
    std::vector<Block*> GetBlocksWithAttribute(const std::string& attribute)
    {
		...
		// Moved this before for loop to avoid creating vectors while parsing through the block
        std::vector<std::string> attributes;
        for (Block* b : m_blocks)
        {            
            attributes = b->GetAttributes();
			// Replaced for loop with std::find from algorithm
            if (std::find(attributes.begin(), attributes.end(), attribute) != attributes.end())
            {
                blocksWithAttribute.emplace_back(b);               
            }
        }
    ...
    bool BlockNameExistsInGraph(const std::string& blockName)
    {
        ...
        {            
			// Avoided creating temporary pointer objects
            std::string iterName = (*i)->GetName();
            if (iterName == blockName)
            {
                exists = true;
				// Return as soon as the we find the duplicate to avoid unnecessary iterations
                return exists;
            }            
        }
        return exists;
	...
	// Since the graph holds the ownership it is the responsibility of Graph class to destroy the block container 
    ~Graph() 
    {
        for (auto& p : m_blocks)
        {
...
private:
    ...
    // Replaced list with vector as it gave better performance output, inspite of frequent insertions 
